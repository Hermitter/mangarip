...
pub struct Scraper<'a> {
    ...
    // link to each chapter in the table of contents
    pub chapter_css_selector: &'a str,

    // link to each img in chapter
...
impl<'a> Fetch for Scraper<'a> {
    ...
    fn chapter_urls(&self) -> Result<Vec<String>, error::ScrapeError> {
        ...
        if !document.is_ok() {
            return Err(error::ScrapeError::UnreadableHtml { url: self.url });
        }
